...
internal sealed class RimTestDoctorRunner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!IsPathDiscoverable(rimContext.CommandPath))
        {
            return Blocked("rimctx", "RIMCONTEXT_COMMAND_NOT_FOUND");
        }
        ...
        if (!IsPathDiscoverable(rimError.CommandPath))
        {
            return Blocked("rimerror", "RIMERROR_COMMAND_NOT_FOUND");
        }
        ...
        RimErrorAdapterOptions rimError)
    {
        ...
        if (!Directory.Exists(rimError.WorkingDirectory))
        {
            return Blocked("rimerror", "RIMERROR_ROOT_NOT_FOUND");
        }
    ...
    private async Task<ProbeResult> ProbeRimContextAsync(
        ...
        CancellationToken cancellationToken)
    {
        var request = new DevBridgeProcessRequest(
            options.CommandPath,
            options.RootPath,
            BuildRimContextArguments(options),
        ...
        {
            return Failure("RIMCONTEXT_CANCELLED");
        }

        if (process.TimedOut)
        {
            return Failure("RIMCONTEXT_CLIENT_TIMEOUT");
        }

        if (!string.IsNullOrWhiteSpace(process.StartError))
        {
            return Failure("RIMCONTEXT_START_FAILED");
        }

        if (process.StdoutTruncated || process.StderrTruncated ||
            string.IsNullOrWhiteSpace(process.Stdout))
        {
            return Failure("RIMCONTEXT_RESPONSE_INVALID");
        }
        ...
        try
        {
            ...
            if (!TryGetString(root, "schemaVersion", out string? schemaVersion) ||
                ...
                !string.Equals(command, "summary", StringComparison.Ordinal) ||
            ...
            {
                string code = TryGetString(root, "code", out string? reportedCode)
                    ? reportedCode!
                    : "RIMCONTEXT_RESPONSE_INVALID";
                return MapRimContextError(code);
            }
    ...
    private static IReadOnlyList<string> BuildRimContextArguments(
        RimContextAdapterOptions options)
    {
    ...
    private void WriteDiagnostic(string component, string? code)
    {
        ...
        {
            stderr.WriteLine($"rimtest doctor: {component} {code}");
        }
    }
